#!/usr/bin/env python3
"""Prepare matched 5x20 visible-options Plain and KIRA-overlap campaigns."""
...
ARMS = ("plain", "kira_overlap")


PLAIN_INSTRUCTIONS = """# Plain Luna visible-options MCQ contract
...
historical benchmark result is available or permitted. Local file operations
may be used only to read input, write output, and validate the output.
...
`{"id":"exact input id","finalAnswer":"option ID only"}`

For example, use `"4"`, not option text or an explanation. Answer every ID
exactly once. Validate JSON, row count, IDs, key sets, order, and that every
answer is one option ID actually present in its prompt before finishing.
"""


KIRA_OVERLAP_INSTRUCTIONS = """# KIRA-inspired Universal-overlap Luna MCQ contract

Answer all 20 questions in `input/questions.jsonl` from your own knowledge.
Select exactly one of the option IDs printed in each prompt. This is one
sealed solver call: no answer key, database, web search, external model,
agent, skill, memory, or historical benchmark result is available or
permitted. Local file operations may be used only to read input, maintain the
work ledger, write output, and validate it.

Use two deliberately different reasoning views for every item. In the first
view, solve the question normally and record a provisional option and
confidence. In the second, restart from the visible options: test each option
against the question, actively seek a counterexample to the provisional
choice, and record the strongest competing option. Do not let the first view
silently anchor the second. Write both views and whether they agree to
`output/work/ledger.jsonl`. For disagreements or low confidence, perform a
focused repair that names the exact deciding fact or rule, then choose one
option. Spend most checking effort on disagreements rather than already
stable items.

After all items, run a global overlap audit: revisit every disagreement and
also the three weakest agreements, looking for negation words, exception
questions, reversed relations, translation ambiguity, and option-number
transcription errors. Record any change in the ledger.

Write exactly 20 UTF-8 JSONL rows to `output/answers.jsonl`, preserving input
order. Every scored row must contain exactly these keys:

`{"id":"exact input id","finalAnswer":"option ID only"}`

For example, use `"4"`, not option text or an explanation. Finish only after
two independent disk checks confirm valid JSON, exactly 20 rows, exact IDs,
key sets and order, and that every final answer is one visible option ID.
"""
...
def prepare(
    *,
    ...
    for arm in ARMS:
        instructions = PLAIN_INSTRUCTIONS if arm == "plain" else KIRA_OVERLAP_INSTRUCTIONS
        for block in selection["blocks"]:
            ...
            freeze = {
                ...
                "protocol": f"visible_options_5x20_solver_v1:{arm}",
                "arm": arm,
                "blockId": block_id,
                "benchmarkId": block["benchmarkId"],
