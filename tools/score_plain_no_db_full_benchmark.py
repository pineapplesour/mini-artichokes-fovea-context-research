...
from tools.score_subject_batch_semantic import (
    ...
    semantic_batch_input_sha256,
)


SCORER_ID = "plain-no-db-full-closed-consensus-v1"
MAX_JUDGE_TOKENS = 60_000
MAX_GROUP_SIZE = 8
TERMINAL_GROUP_DISPOSITIONS = {"scored", "judge_unresolved"}


def judge_attempt_errors(attempt: dict[str, Any], *, rows_by_id: dict[str, dict[str, Any]], judge_model: str) -> list[str]:
    errors: list[str] = []
    case_ids = [str(value) for value in attempt.get("caseIds", [])]
    rows = [rows_by_id[case_id] for case_id in case_ids if case_id in rows_by_id]
    if len(rows) != len(case_ids) or not rows:
        errors.append("judge_attempt_case_ids_invalid")
        return errors
    if attempt.get("policyVersion") != POLICY_VERSION:
        errors.append("judge_policy_changed")
    if attempt.get("semanticBatchInputSha256") != semantic_batch_input_sha256(rows=rows, judge_model=judge_model):
        errors.append("judge_input_changed")
    trace = attempt.get("modelTrace")
    trace_audit = _audit_closed_judge_trace(trace)
    if attempt.get("closedToolTraceAudit") != trace_audit or trace_audit.get("passed") is not True:
        errors.append("judge_trace_invalid")
    usage = trace.get("tokenUsage") if isinstance(trace, dict) and isinstance(trace.get("tokenUsage"), dict) else {}
    if int(usage.get("totalTokens") or 0) > MAX_JUDGE_TOKENS:
        errors.append("judge_token_budget_exceeded")
    if not attempt.get("error"):
        judgments = attempt.get("judgments")
        if not isinstance(judgments, list) or [str(row.get("id") or "") for row in judgments] != case_ids:
            errors.append("judge_judgments_invalid")
    return errors


def group_id(*, benchmark_id: str, case_ids: list[str], judge_model: str) -> str:
    return "judge-" + canonical_json_sha256(
        {
            "scorerId": SCORER_ID,
...
def partition_rows(rows: list[dict[str, Any]]) -> list[list[dict[str, Any]]]:
    return [rows[index : index + MAX_GROUP_SIZE] for index in range(0, len(rows), MAX_GROUP_SIZE)]


def collect_unresolved_semantic_rows(
    *,
    ...
    """Collect every live semantic case before partitioning it into bounded judge groups."""
    ...
    for case in deterministic_report.get("cases", []):
        ...
        if case_id not in public_by_id or case_id not in private_by_id:
            raise ValueError(f"semantic grading manifest binding failed: {case_id}")
        result_path = results_dir / f"{case_id}.json"
        result = json.loads(result_path.read_text(encoding="utf-8"))
        if str(result.get("error") or "").strip():
            continue
        candidate = str(result.get("prediction") or "").strip()
        if not candidate:
            raise ValueError(f"semantic grading candidate is empty: {case_id}")
...
def judge_artifact_digest(payload: dict[str, Any]) -> str:
    return canonical_json_sha256({key: value for key, value in payload.items() if key != "artifactDigest"})
...
def validate_judge_artifact(
    ...
    *,
    ...
    expected_dispositions = TERMINAL_GROUP_DISPOSITIONS if terminal else {"in_progress"}
    if (
        artifact.get("schemaVersion") != SCHEMA_VERSION
        or artifact.get("groupId") != group_id_value
        or artifact.get("benchmarkId") != benchmark_id
        or artifact.get("caseIds") != case_ids
        or artifact.get("disposition") not in expected_dispositions
    ):
        raise ValueError(f"judge group artifact identity mismatch: {group_id_value}")
    if artifact.get("artifactDigest") != judge_artifact_digest(artifact):
        raise ValueError(f"judge group digest changed: {group_id_value}")
    attempts = artifact.get("attempts")
    if not isinstance(attempts, list) or len(attempts) > 3:
        raise ValueError(f"judge group attempts invalid: {group_id_value}")
    seen_repeats: set[int] = set()
    rows_by_id = {str(row["id"]): row for row in rows}
    for attempt in attempts:
        if not isinstance(attempt, dict):
            raise ValueError(f"judge group attempt is not an object: {group_id_value}")
        repeat = int(attempt.get("repeat") or 0)
        attempt_ids = [str(value) for value in attempt.get("caseIds", [])]
        if repeat not in {1, 2, 3} or repeat in seen_repeats:
            raise ValueError(f"judge group repeat invalid: {group_id_value}")
        if repeat in {1, 2} and attempt_ids != case_ids:
            raise ValueError(f"judge group primary repeat coverage changed: {group_id_value}")
        if repeat == 3 and (not attempt_ids or any(case_id not in rows_by_id for case_id in attempt_ids)):
            raise ValueError(f"judge group tie-break coverage changed: {group_id_value}")
        if judge_attempt_errors(attempt, rows_by_id=rows_by_id, judge_model=judge_model):
            raise ValueError(f"judge group attempt changed: {group_id_value}")
        seen_repeats.add(repeat)
    if terminal and not {1, 2}.issubset(seen_repeats):
        raise ValueError(f"judge group terminal artifact lacks primary repeats: {group_id_value}")
    return list(attempts)


def write_partial_judge_artifact(
    path: Path,
    *,
    group_id_value: str,
    benchmark_id: str,
    ...
    partial["artifactDigest"] = judge_artifact_digest(partial)
...
def validate_solver_outputs(
    *,
    ...
    if ledger.get("complete") is not True or int(ledger.get("totals", {}).get("terminalCases") or 0) != EXPECTED_READY_CASES:
        raise ValueError("solver ledger is not complete")
    if ledger.get("freezeSha256") != sha256_file(freeze_path):
        raise ValueError("solver ledger freeze mismatch")
    ...
    if len(records_by_id) != len(batches):
        raise ValueError("solver ledger batch coverage mismatch")
    ...
    for batch in batches:
        ...
        if not record or record.get("disposition") not in TERMINAL_DISPOSITIONS:
            raise ValueError(f"solver batch is not terminal: {batch.batch_id}")
        ...
        if not artifact_path.is_file() or record.get("artifactSha256") != sha256_file(artifact_path):
            raise ValueError(f"solver batch artifact changed: {batch.batch_id}")
        ...
        for case_id in batch.case_ids:
            path = solver_root / "results" / f"{case_id}.json"
            if not path.is_file():
                raise ValueError(f"solver case result missing: {case_id}")
            result = json.loads(path.read_text(encoding="utf-8"))
            if result.get("batchId") != batch.batch_id or result.get("batchArtifactDigest") != artifact.get("artifactDigest"):
                raise ValueError(f"solver case result binding failed: {case_id}")
            ...
            if (
                ...
                or result.get("mode") != artifact.get("mode")
                or result.get("model") != artifact.get("model")
                or result.get("prediction") != artifact.get("predictions", {}).get(case_id)
                or str(result.get("error") or "") != expected_error
                or result.get("closedToolAuditPassed") is not (artifact.get("closedToolAudit", {}).get("passed") is True)
            ):
                raise ValueError(f"solver case result content changed: {case_id}")
            ...
            if not boundary or any(value is not False for value in boundary.values()):
                raise ValueError(f"solver case result tool boundary changed: {case_id}")
            result_count += 1
...
def deterministic_reports(
    *,
    ...
    for entry in registry.get("entries", []):
        ...
        output = score_root / "deterministic" / f"{canonical_json_sha256(benchmark_id)[:16]}.json"
        if persist:
    ...
    if sum(int(item["report"].get("total") or 0) for item in reports) != EXPECTED_READY_CASES:
        raise ValueError("deterministic scoring coverage mismatch")
    return reports


def finalize_group(*, deterministic: dict[str, Any], rows: list[dict[str, Any]], attempts: list[dict[str, Any]], judge_model: str) -> dict[str, Any]:
    subset_ids = {str(row["id"]) for row in rows}
    subset_deterministic = {
        **deterministic,
        "cases": [case for case in deterministic.get("cases", []) if str(case.get("caseId") or "") in subset_ids],
    }
    return finalize_semantic_batch(
        deterministic_report=subset_deterministic,
        rows=rows,
        attempts=attempts,
...
def ledger_totals(records: list[dict[str, Any]], target_groups: int) -> dict[str, int]:
    terminal = [record for record in records if record.get("disposition") in TERMINAL_GROUP_DISPOSITIONS]
    return {
        "targetGroups": target_groups,
        "terminalGroups": len(terminal),
        "pendingGroups": target_groups - len(terminal),
        "semanticCases": sum(len(record.get("caseIds") or []) for record in terminal),
        "judgeCalls": sum(int(record.get("judgeCalls") or 0) for record in terminal),
        "judgeTokens": sum(int(record.get("judgeTokens") or 0) for record in terminal),
    }
...
def run_scoring(
    *,
    ...
    if score_ledger_path.exists():
        ...
        for field, expected in (
            ...
            if score_ledger.get(field) != expected:
                raise ValueError(f"score ledger mismatch: {field}")
    records_by_id = {
    ...
    if len(records_by_id) != len(score_ledger.get("records", [])):
        raise ValueError("score ledger contains duplicate group IDs")
    ...
    for index, spec in enumerate(group_specs, start=1):
        ...
        if record and record.get("disposition") in TERMINAL_GROUP_DISPOSITIONS:
            if not artifact_path.is_file() or record.get("artifactSha256") != sha256_file(artifact_path):
                raise ValueError(f"judge group artifact changed: {gid}")
            artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
        ...
        if dry_run or (maximum_new_groups > 0 and new_groups >= maximum_new_groups):
            continue
        if client is None:
            raise RuntimeError("judge client is unavailable")
        attempts: list[dict[str, Any]] = []
        ...
        for repeat in (1, 2):
            ...
            attempt_errors = judge_attempt_errors(attempt, rows_by_id=rows_by_id, judge_model=judge_model)
            if attempt_errors:
                raise RuntimeError(f"judge attempt failed closed integrity checks: {gid}: {','.join(attempt_errors)}")
        ...
        primary_attempts = [attempt for attempt in attempts if int(attempt.get("repeat") or 0) in {1, 2}]
        preliminary = finalize_group(
            deterministic=spec["deterministic"], rows=rows, attempts=primary_attempts, judge_model=judge_model
        )
        unresolved_ids = {
            str(case["caseId"])
        ...
        if unresolved_ids:
            ...
            if existing_third is not None:
                if [str(value) for value in existing_third.get("caseIds", [])] != [str(row["id"]) for row in third_rows]:
                    raise ValueError(f"judge group resumed tie-break set changed: {gid}")
            else:
                attempt = run_semantic_batch_attempt(rows=third_rows, repeat=3, judge_client=client, judge_model=judge_model)
                third_map = {str(row["id"]): row for row in third_rows}
                attempt_errors = judge_attempt_errors(attempt, rows_by_id=third_map, judge_model=judge_model)
                if attempt_errors:
                    raise RuntimeError(f"judge tie-break failed closed integrity checks: {gid}: {','.join(attempt_errors)}")
                attempts.append(attempt)
                write_partial_judge_artifact(
                    artifact_path,
                    group_id_value=gid,
            ...
            raise ValueError(f"judge group contains an unnecessary tie-break: {gid}")
        final = finalize_group(deterministic=spec["deterministic"], rows=rows, attempts=attempts, judge_model=judge_model)
    ...
    if score_ledger["complete"] and not dry_run:
        ...
        for item in reports:
            ...
            cases = [semantic_by_id.get(str(case["caseId"]), case) for case in deterministic.get("cases", [])]
            total = len(cases)
            ...
            write_json(output, report)
            final_by_benchmark[item["benchmarkId"]] = {**report, "path": str(output.resolve()), "sha256": sha256_file(output)}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Deterministically and independently grade the full closed no-DB Plain run.")
