#!/usr/bin/env python3
"""Freeze the complete official Aider Go track with tests hidden."""
...
def run(argv: list[str], *, cwd: Path) -> str:
    completed = subprocess.run(
        argv, cwd=cwd, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False
    )
...
def task_instruction(task_dir: Path, solution_files: list[str]) -> str:
    ...
    parts.append(
        ...
        f"{' '.join(solution_files)}. Preserve all declared package names, exported identifiers, and signatures. "
        "Use Go 1.22 and only the standard library or dependencies already declared by the exercise. "
...
def materialize(source: Path, output: Path, go_root: Path) -> dict[str, object]:
    if output.exists():
    ...
    root = source / "go" / "exercises" / "practice"
    selected = sorted(path for path in root.iterdir() if path.is_dir())
    if len(selected) != 39:
        raise ValueError(f"expected complete Go track of 39 tasks, found {len(selected)}")
    ...
    for original in selected:
        ...
        conventional_tests = {
            path.relative_to(original).as_posix() for path in original.rglob("*_test.go") if path.is_file()
        }
        test_files = sorted(declared_tests | conventional_tests)
        relative = Path("tasks") / "go" / original.name
        ...
        item = {
            "taskId": f"aider-go/{original.name}",
            "language": "go",
            "name": original.name,
            ...
            "officialTree": run(
                ["git", "rev-parse", f"HEAD:go/exercises/practice/{original.name}"], cwd=source
            ),
    ...
    manifest = {
        ...
        "benchmark": "Aider-AI/polyglot-benchmark complete hidden-test Go track",
        "officialRepository": "https://github.com/Aider-AI/polyglot-benchmark",
        "officialCommit": commit,
        "selectionRule": "all 39 official Go exercises; no task selection",
    ...
    (agent_source / ".gitignore").write_text("**/*.test\n", encoding="utf-8")
    combined = "\n\n".join(
        f"## {item['taskId']}\nSolution files: {' '.join(item['solutionFiles'])}\n\n{item['instruction']}"
        for item in tasks
    )
    instruction = (
        "Solve all 39 independent official Aider Go benchmark exercises in the frozen order below. "
        "Official tests and gold implementations are unavailable. Work only in listed solution files, infer edge "
        "cases from the official instructions, and do not search for exercise solutions. Do not edit tests, go.mod, "
        "go.sum, benchmark_manifest.json, .docs files, or repository metadata.\n\n" + combined
    )
    ...
    run(["git", "commit", "--quiet", "-m", "Freeze complete hidden-test Aider Go track"], cwd=agent_source)
    base_ref = run(["git", "rev-parse", "HEAD"], cwd=agent_source)
    evaluator = (Path(__file__).resolve().parent / "evaluate_aider_hidden_go_full.py").resolve()
    private_root = (output / "private").resolve()
    task_payload = {
        "task_id": "aider-hidden-go39-v1",
        "instruction": instruction,
        "base_ref": base_ref,
        "public_test_command": (
            f"python {json.dumps(str(evaluator))} --workspace . --private-root "
            f"{json.dumps(str(private_root))} --go-root {json.dumps(str(go_root))}"
        ),
        "allowed_path_globs": allowed,
        "forbidden_path_globs": [
            ".git/*", "benchmark_manifest.json", ".gitignore", "**/.docs/*", "**/*_test.go",
            "**/go.mod", "**/go.sum",
        ],
    }
...
def main() -> int:
    ...
    print(json.dumps(materialize(args.source, args.output, args.go_root)))
    return 0
